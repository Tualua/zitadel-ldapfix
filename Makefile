...
compile_pipeline: console_move
	CGO_ENABLED=0 go build -o zitadel -v -ldflags="-s -w -X 'github.com/Tualua/zitadel-ldapfix/cmd/build.commit=$(COMMIT_SHA)' -X 'github.com/Tualua/zitadel-ldapfix/cmd/build.date=$(now)' -X 'github.com/Tualua/zitadel-ldapfix/cmd/build.version=$(VERSION)' "
	chmod +x zitadel
...
core_api: core_api_generator core_grpc_dependencies
	buf generate
	mkdir -p pkg/grpc
	cp -r .artifacts/grpc/github.com/Tualua/zitadel-ldapfix/pkg/grpc/** pkg/grpc/
...
core_integration_reports:
	go tool covdata textfmt -i=tmp/coverage -pkg=github.com/Tualua/zitadel-ldapfix/internal/...,github.com/Tualua/zitadel-ldapfix/cmd/... -o profile.cov
