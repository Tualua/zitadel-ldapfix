...
Download the ZITADEL release according to your architecture from [Github](https://github.com/Tualua/zitadel-ldapfix/releases/latest)
...
LATEST=$(curl -i https://github.com/Tualua/zitadel-ldapfix/releases/latest | grep location: | cut -d '/' -f 8 | tr -d '\r'); wget -qO-  https://github.com/Tualua/zitadel-ldapfix/releases/download/$LATEST/zitadel_Darwin_$(uname -m).tar.gz  | tar -xJ zitadel && sudo mv zitadel /usr/local/bin
```
